...
class WebBundleBuildTests(unittest.TestCase):
    def test_build_is_deterministic_and_rooted_at_static_contents(self):
        with tempfile.TemporaryDirectory() as work:
            root, static = Path(work), Path(work) / "static"
            ...
            one = (root / "one" / f"{first['bundleVersion']}.zip").read_bytes()
            two = (root / "two" / f"{second['bundleVersion']}.zip").read_bytes()
            self.assertEqual(first, second)
            self.assertEqual(one, two)
            self.assertEqual(hashlib.sha256(one).hexdigest(), first["sha256"])
...
class WebBundleRouteTests(unittest.TestCase):
    def setUp(self):
        self.bundle_temp, self.config_temp = tempfile.TemporaryDirectory(), tempfile.TemporaryDirectory()
        os.environ["GALIB_APP_BUNDLE_DIR"] = self.bundle_temp.name
        os.environ["MANGA_DL_CONFIG_DIR"] = self.config_temp.name
        os.environ["MANGA_DL_PASSWORD"] = "secret"
        import app
        self.bundle_module = importlib.reload(web_bundle)
        self.app_module = importlib.reload(app)
        self.manifest = self.bundle_module.build_bundle(Path(self.app_module.STATIC_DIR), Path(self.bundle_temp.name))
        self.client = self.app_module.app.test_client()

    def tearDown(self):
        self.bundle_temp.cleanup(); self.config_temp.cleanup()
        os.environ.pop("GALIB_APP_BUNDLE_DIR", None); os.environ.pop("MANGA_DL_PASSWORD", None)
    ...
    def test_manifest_archive_and_cache_contract(self):
        self.login()
        with self.client.get("/api/app-bundle/manifest") as response:
